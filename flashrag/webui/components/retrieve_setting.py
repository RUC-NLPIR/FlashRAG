...
def create_retrieve() -> Dict[str, "Component"]:
    with gr.Accordion(open = False) as retrieve_tab:
        with gr.Row():
            ...
            retrieval_topk = gr.Slider(minimum = 1, maximum = 100, value = 10, step = 1, interactive = True)
            retrieval_use_fp16 = gr.Checkbox(value = True, interactive = True)
            retrieval_pooling_method = gr.Dropdown(choices = ['mean', 'pooling', 'cls'], value = "mean", interactive = True)
        with gr.Row():
            query_max_length = gr.Slider(minimum = 1, maximum = 2048, value = 512, step = 1, interactive = True)
            retrieval_batch_size = gr.Slider(minimum = 1, maximum = 1024, value = 256, step = 1, interactive = True)
        with gr.Row():
            bm25_backend = gr.Dropdown(choices = ["BM25s", "Pyserini"], value = "Pyserini", interactive = True)
