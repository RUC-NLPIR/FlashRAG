    ...
    def get_reasoning_chain(self, all_query, all_doc_triples, triple_to_doc_ids):
        ...
        for query, doc_triples, generating_chain_examplars, final_chain_examplars, triple_doc_id_list in tqdm(
            ...
            for j in range(self.max_chain_length):
                ...
                topk_most_relevant_triples_indices = torch.topk(path_triples_similarities, k=min(self.topk_triple_select, num_total_triples), dim=1)[1].tolist()
                ...
                topk_choices_probs, topk_choices_indices = torch.topk(answer_token_probs, k=self.num_beams, dim=1) # for each path, select choice token in topk probs
                for i in range(len(paths)):
                    ...
                    for b in range(self.num_beams):
                        if torch.isnan(topk_choices_probs[i, b]) or topk_choices_probs[i, b].item() < self.min_triple_prob:
                            continue
