        ...
        for idx in trange(0, len(input_list), batch_size, desc="Generation process: "):
            ...
            if self.fid:
                ...
                inputs = self.tokenizer(
                    batched_prompts, return_tensors="pt", padding=True, truncation=True, max_length=self.max_input_len
                ).to(self.device)
    ...
    def generate(self, input_list: List[str], return_raw_output=False, return_scores=False, **params):
        ...
        if self.use_lora:
            ...
            outputs = self.model.generate(
                input_list, sampling_params, lora_request=LoRARequest("lora_module", 1, self.lora_path)
            )
        else:
...
class HFCausalLMGenerator(BaseGenerator):
    ...
    def generate(self, input_list: List[str], batch_size=None, return_scores=False, return_dict=False, **params):
        ...
        for idx in trange(0, len(input_list), batch_size, desc="Generation process: "):
            ...
            outputs = self.model.generate(
                **inputs, output_scores=True, return_dict_in_generate=True, **generation_params
            )
            ...
            if return_dict:
                ...
                batch_generated_token_logits = (
                    torch.cat([token_scores.unsqueeze(1) for token_scores in outputs.scores], dim=1).detach().cpu()
                )
                if batch_generated_token_ids.shape[1] < generation_params["max_new_tokens"]:
                    ...
                    padding_token_ids = torch.zeros(
                        (real_batch_size, padding_length), dtype=batch_generated_token_ids.dtype
                    ...
                    batch_generated_token_logits = torch.cat(
                        [batch_generated_token_logits, padding_token_logits], dim=1
                    )
            ...
            for i, generated_sequence in enumerate(outputs.sequences):
                ...
                text = self.tokenizer.decode(
                    generated_sequence, skip_special_tokens=True, clean_up_tokenization_spaces=False
                )
